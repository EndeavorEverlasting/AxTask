...
function scrubSnippet(text: string, maxLen: number): string {
  let s = text;
  // Strip URLs first (http(s), protocol-relative, www., common shorteners, bare host-like paths).
  s = s.replace(/\bhttps?:\/\/[^\s]+/gi, " ");
  s = s.replace(/\b\/\/[^\s]+/g, " ");
  s = s.replace(/\b(?:www\.)[^\s]+/gi, " ");
  s = s.replace(
    /\b(?:t\.co|bit\.ly|goo\.gl|tinyurl\.com|ow\.ly|buff\.ly|youtu\.be|short\.link)\/[^\s]*/gi,
    " ",
  );
  s = s.replace(/\b[A-Za-z0-9][-A-Za-z0-9]*\.(?:com|net|org|io|co|app|dev)\b(?:\/[^\s]*)?/gi, " ");
  s = s.replace(/\S+@\S+\.\S+/g, " ");
