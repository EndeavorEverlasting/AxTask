 ...
 * (Compose profile `nodeweaver` builds from **`services/nodeweaver/upstream`**, or use any
...
export async function callNodeWeaverBatchClassify(
  ...
  try {
    ...
    throw new Error(
      `NodeWeaver classify: invalid JSON (status ${response.status}, content-type ${ct || "none"}): ${
