...
describe("AI harness infrastructure completeness", () => {
  ...
  it("maps repository commands, configurations, and known traps", () => {
    ...
    for (const id of ["install", "development", "typecheck", "test", "build", "deploy-contract", "production-start"]) {
  ...
  it("routes validator and workflow failures to a scoped recovery procedure", () => {
    ...
    expect(workflows.workflows).toContainEqual(
      expect.objectContaining({
        ...
        path: ".ai/workflows/failure-recovery.md",
      }),
    );
    expect(triggers.triggers).toContainEqual(
      expect.objectContaining({
        id: "validator-or-workflow-failed",
        workflowId: "axtask.failure-recovery.v1",
      }),
    );
  ...
  it("keeps operator failure output human-readable and sanitized", () => {
    const report = fs.readFileSync(
      path.join(REPO_ROOT, ".ai", "reports", "failure-report-template.md"),
      "utf8",
    );
