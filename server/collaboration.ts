...
const joinTaskSchema = z.object({ type: z.literal("join_task"), taskId: z.string().min(1) });
const leaveTaskSchema = z.object({ type: z.literal("leave_task") });
const focusFieldSchema = z.object({
  type: z.literal("focus_field"),
  field: z.string(),
  cursorPosition: z.number().optional(),
});
const blurFieldSchema = z.object({ type: z.literal("blur_field") });
const fieldEditSchema = z.object({
  type: z.literal("field_edit"),
  field: z.string(),
  value: z.string(),
});
...
const collabMessageSchema = z.discriminatedUnion("type", [
  joinTaskSchema,
  leaveTaskSchema,
  focusFieldSchema,
  blurFieldSchema,
...
function parseCollabMessage(raw: string): CollabMessage | null {
  let parsed: unknown;
  try {
  ...
  const out = collabMessageSchema.safeParse(parsed);
  return out.success ? (out.data as CollabMessage) : null;
}
...
export function setupCollaborationWs(server: Server) {
  ...
  wss.on("connection", (ws: WebSocket, _req: IncomingMessage, user: { userId: string; email: string; displayName: string | null }) => {
    ...
    ws.on("message", async (data) => {
      try {
        const msg = parseCollabMessage(data.toString());
        if (!msg) return;
