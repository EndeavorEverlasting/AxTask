...
function remapRow(row: BundleRow, tableName: TableName, pkField: string, idMap: Map<string, string>): BundleRow {
  ...
  for (const rule of fkRules) {
    ...
    if (val) {
      ...
      // Non-nullable FK with no idMap entry: keep original id (may still resolve if row
      // exists in DB, or fail insert with FK violation — intentional for partial bundles).
    }
  }
