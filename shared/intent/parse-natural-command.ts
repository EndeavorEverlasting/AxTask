/**
 * Natural-language command parser: detector order is navigation, alarm list,
 * task review, planning, search, then create task/reminder/recurrence. When the
 * server dispatches, calendar/shopping/review can override; see
 * `mapParsedCommandToIntent` in `server/engines/dispatcher.ts`.
 */
import {
  type ParsedCommand,
...
function detectNavigation(normalized: string): ParsedCommand | null {
  ...
  const targets: Array<[RegExp, string]> = [
    [/\b(?:open|show|go to|navigate to|switch to|take me to)\s+(?:the\s+)?(?:dashboard|home)\b/, "/"],
    [/\b(?:open|show|go to|navigate to|switch to|take me to)\s+(?:the\s+)?(?:calendar)\b/, "/calendar"],
    [/\b(?:open|show|go to|navigate to|switch to|take me to)\s+(?:the\s+)?(?:planner|ai planner)\b/, "/planner"],
    [/\b(?:open|show|go to|navigate to|switch to|take me to)\s+(?:the\s+)?(?:tasks?|task list|all tasks)\b/, "/tasks"],
    [/\b(?:open|show|go to|navigate to|switch to|take me to)\s+(?:the\s+)?(?:analytics)\b/, "/analytics"],
    [/\b(?:open|show|go to|navigate to|switch to|take me to)\s+(?:the\s+)?(?:checklist)\b/, "/checklist"],
    [/\b(?:open|show|go to|navigate to|switch to|take me to)\s+(?:my\s+)?(?:shopping|grocery)(?:\s+list)?\b/, "/shopping"],
    [/\b(?:open|show|go to|navigate to|switch to|take me to)\s+(?:the\s+)?(?:alarms?|reminders?)\b/, "/alarms"],
  ];
...
function detectTaskReview(normalized: string): ParsedCommand | null {
  ...
  let activity = normalized
    ...
    .replace(/\s{2,}/g, " ")
    .trim();
  activity = activity.replace(/\s+done$/i, "").replace(/\s{2,}/g, " ").trim();
...
function detectTaskOrReminder(normalized: string, context: ParseCommandContext): ParsedCommand | null {
  ...
  if (recurrence.recurrence === "irregular") {
    warnings.push("Irregular recurrence needs product rules before auto-scheduling.");
  }
...
export function parseNaturalCommand(raw: string, context: ParseCommandContext): ParsedCommand {
  ...
  const detectors = [detectNavigation, detectAlarmList, detectTaskReview, detectPlanning, detectSearch] as const;
