 ...
 *
 * Privacy: never echoes SESSION_SECRET / INVITE_CODE / AUTH_AUDIT_PEPPER to the terminal. Operational
...
function parseArgs(argv) {
  ...
  for (const a of argv) {
    ...
    else if (a === "--placeholders-only") out.placeholdersOnly = true;
...
Writes a gitignored .env.render (from .env.render.example) with:
  ...
  • AUTH_AUDIT_PEPPER (cryptographically random, ≥16 chars; required on Render in production)
  • Optional: CANONICAL_HOST / BASE_URL / redirect hints from --domain

With --placeholders-only: same file shape but keeps template placeholders (no random secrets).
Use that for agents/CI when real secrets are unavailable.
...
Options:
  ...
  --invite          Set REGISTRATION_MODE=invite and a random INVITE_CODE (ignored with --placeholders-only)
  --placeholders-only   Do not generate SESSION_SECRET / AUTH_AUDIT_PEPPER / INVITE_CODE; keep .env.render.example placeholders
  --refresh-secrets-only   Only update SESSION_SECRET, AUTH_AUDIT_PEPPER, and INVITE_CODE (if present) in existing .env.render
  --help, -h        This message

Examples:
  npm run render:env-bootstrap -- --domain=axtask.app --invite
  npm run render:env-placeholder -- --domain=axtask.app
...
function refreshSecretsInFile(filePath) {
  ...
  next = replaceLine(next, /^\s*AUTH_AUDIT_PEPPER\s*=/, `AUTH_AUDIT_PEPPER=${newAuthAuditPepper()}`);
...
function printPasteGuide(domain) {
  ...
  const lines = [
    ...
    `Manage → Environment  = paste env vars (DATABASE_URL, SESSION_SECRET, AUTH_AUDIT_PEPPER, …).`,
...
async function main() {
  ...
  if (opts.placeholdersOnly && opts.refreshSecretsOnly) {
    logInfo("[render:env-bootstrap] Cannot combine --placeholders-only with --refresh-secrets-only.");
    process.exit(1);
  }
  ...
  if (opts.refreshSecretsOnly) {
    ...
    if (opts.stdout) {
      ...
      logInfo(
        "[render:env-bootstrap] Refreshed SESSION_SECRET, AUTH_AUDIT_PEPPER" +
          (/(^|\n)INVITE_CODE=/m.test(next) ? ", INVITE_CODE" : "") +
          " in " +
          opts.outPath,
      );
    }
  ...
  const bannerMode = opts.placeholdersOnly
    ? "placeholders-only (no random secrets; safe for agents/CI)"
    : "full bootstrap";
  const banner = `# -----------------------------------------------------------------------------\n# Generated by tools/render/render-env-bootstrap.mjs — ${new Date().toISOString().slice(0, 10)} (${bannerMode})\n# Regenerate secrets only: npm run render:env-bootstrap -- --refresh-secrets-only --force\n# -----------------------------------------------------------------------------\n\n`;
  ...
  if (opts.placeholdersOnly) {
    if (opts.invite) {
      logInfo(
        "[render:env-bootstrap] --invite ignored with --placeholders-only (no random INVITE_CODE generated).",
      );
    }
  } else {
    text = text.replace(/^SESSION_SECRET=.*$/m, `SESSION_SECRET=${newSessionSecret()}`);
    text = text.replace(/^#?\s*AUTH_AUDIT_PEPPER=.*$/m, `AUTH_AUDIT_PEPPER=${newAuthAuditPepper()}`);
