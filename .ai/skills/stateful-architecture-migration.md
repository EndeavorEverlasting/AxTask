...
2. Run `node scripts/ai-harness/next-stateful-task.mjs` and accept exactly the one routed surface/gap as the current reasoning boundary. Do not use or invent a manual surface override.
...
5. Update only the current gap in `.ai/architecture/surfaces/<surface-id>.json`. Evidence must cite an existing routed exact file and may not exceed the surface `proofCeiling`.
6. Set the current gap to `resolved` only with concrete source, finding, and proof level; set the artifact's top-level status to the next status emitted by the router; then run the exact `--require=<gap-id>` validator.
7. Run the router again. Never manually jump to a later surface while an earlier open or blocked gap remains.
...
9. Record the canonical decision and set the per-surface task status to `COMPLETED`, then run both decision validators. Per-surface `COMPLETED` means the evidence/decision routing loop is closed; it does not mean a migration was implemented.
10. Set exactly one non-`keep` canonical surface to `decisionStatus=approved` only when the canonical migration guardrails are satisfied. `approved` is active authorization, not historical status.
11. If approved for migration, execute one seam only. After required proof is complete, set canonical `decisionStatus=completed` before another non-`keep` seam may be approved.
...
- Evidence source must be inside the routed exact-file boundary.
- Evidence proof level must not exceed the current surface proof ceiling.
- Per-surface `COMPLETED` closes routing only; canonical `completed` is historical migration evidence.
- `approved` means active authorization; canonical `completed` does not authorize new mutation.
- At most one non-`keep` canonical surface may be `approved` at a time.
...
Contract/harness validation proves the task loop and migration rules are enforced. Static tests prove code contracts. Build proves buildability. Launcher/browser proof requires the real launcher/browser surface. Behavior observed requires actual observation. Live runtime proof requires the protected live environment. Never promote proof above the routed surface ceiling.
