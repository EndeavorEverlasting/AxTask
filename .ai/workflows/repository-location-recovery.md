...
5. If no checkout path is currently known on Windows but GitHub access is available, download only the reviewed operator bootstrap from immutable revision `a50fa0c1353ed2e0a9f45c3da112a0bd4d03493b`. `-EnsureArtifactWorktree` requires `-Fetch`; it reuses an already-discovered exact-`origin/main` usable checkout when possible and otherwise creates a durable sibling detached worktree at that exact SHA. The temporary bootstrap file may not own sprint state:
