...
The queue never overrides `AGENTS.md`, `AGENT_GUARDRAILS.md`, `.ai/authority.json`, executable contracts, or current repository/runtime evidence. Before acting on a queue item, reconcile it against current `main`, open PRs, CI, and the referenced files. If reality has moved, update this ledger first.
...
9. **DONE is strict.** `DONE` means the acceptance gate is satisfied and **no safe actionable work remains inside the item's scope**. `Last proof` must identify durable evidence (merge SHA, passing workflow/run, artifact, or operator proof) and `Next action` must be exactly `none; no safe actionable work remains`.
...
- `OPERATOR` — next progress requires human-controlled credentials, production/runtime access, explicit approval, or another operator-only action.
...
Every `AXQ-*` task block must keep these fields. Add concise links/paths instead of copying long implementation notes into this file.
...
- **Last proof:** disposable PostgreSQL account-evidence certification passed before PR #115 merged; no production evidence bundle exists
...
- **Owner:** unclaimed
- **Branch / PR:** none
- **Scope:** reconcile current Render service configuration with current `main`, resume only when database recovery gates allow it, deploy an exact validated commit, and capture live health/startup proof
- **Forbidden:** repeated blind resumes/deploys, bypassing R1.5/R3, assuming `render.yaml` automatically governs the existing manually configured Docker service, exposing secrets
...
- **Acceptance gate:** exact intended `main` commit is deployed once under verified environment/health settings; startup gates and Render health succeed; live proof is recorded without secrets
- **Gate:** Render remains intentionally suspended until the production database preservation/recovery sequence permits resume
- **Last proof:** repository CI/local production certification is green for the merged preservation sprint; no current live Render proof exists
- **Next action:** after AXQ-004 closes the database recovery gate, reconcile Render environment/health/auto-deploy settings against current `main`, then perform one controlled exact-commit deployment and capture logs/health proof
- **Updated:** 2026-08-09
