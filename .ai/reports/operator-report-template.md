...
When runtime or deployment behavior was exercised, record the sanitized `.ai/runs/<run-id>/runtime-proof.json` and sibling `.ai/runs/<run-id>/local-cert-report.md` references, validator result, attained proof level, and proof ceiling. State `not produced` when the sprint did not execute a runtime workflow. Never promote local-runtime proof to live deployment or operator acceptance.
