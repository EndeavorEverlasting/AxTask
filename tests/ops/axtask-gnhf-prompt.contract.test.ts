...
const directGnhfInvocation = /^\s*(?:&\s+)?(?:gnhf(?:\.ps1|\.cmd)?|\$gnhf(?:Path|Command|Launcher)?)\s+(?:`\s*)?--agent\b/im;

describe("AxTask DeepSeek GNHF harness", () => {
  it("passes the deterministic harness validator", () => {
    const result = spawnSync(process.execPath, [validatorPath], { cwd: repoRoot, encoding: "utf8", env: { ...process.env } });
  ...
  it("keeps the runtime objective separate from launch logic", () => {
    ...
    expect(prompt).toContain("Provider authentication, quota, rate limit, network, model discovery, spawn timeout");
  ...
  it("enters the repository before Git or repair execution", () => {
    ...
    expect(directoryIndex).toBeLessThan(launcher.indexOf("& pwsh -NoLogo -NoProfile -ExecutionPolicy Bypass -File $RepairScript -Apply"));
