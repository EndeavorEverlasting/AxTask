...
function buildPredicate(tableName, columns) {
  ...
  for (const column of DIRECT_ACCOUNT_COLUMNS) {
    if (columns.has(column)) {
      clauses.push(`${quoteIdent(column)} = current_setting('axtask.evidence_user_id')`);
    }
  }
  if (columns.has("task_id")) {
    clauses.push(`${quoteIdent("task_id")} IN (SELECT id FROM public.tasks WHERE user_id = current_setting('axtask.evidence_user_id'))`);
  }
  if (columns.has("invoice_id")) {
    clauses.push(`${quoteIdent("invoice_id")} IN (SELECT id FROM public.invoices WHERE user_id = current_setting('axtask.evidence_user_id'))`);
  }
  if (columns.has("asset_id")) {
    clauses.push(`${quoteIdent("asset_id")} IN (SELECT id FROM public.attachment_assets WHERE user_id = current_setting('axtask.evidence_user_id'))`);
  }
  return clauses.length > 0 ? `(${clauses.join(" OR ")})` : null;
}

function orderByFor(columns) {
  if (columns.has("created_at") && columns.has("id")) return " ORDER BY created_at, id";
...
async function resolveAccount(client, args) {
  ...
  if (Boolean(userId) === Boolean(email)) {
    throw new Error("provide exactly one selector: --user-id=<uuid> or --email=<address>");
  }
  const result = userId
    ? await client.query(
  ...
  if (result.rowCount !== 1) throw new Error("account selector did not resolve exactly one user");
...
async function exportQueryToJsonl(client, options) {
  ...
  try {
    await client.query(`DECLARE ${quoteIdent(cursorName)} NO SCROLL CURSOR FOR ${querySql}`);
    while (true) {
      const batch = await client.query(`FETCH FORWARD ${batchSize} FROM ${quoteIdent(cursorName)}`);
...
async function buildApiRequestSummary(client, predicate) {
  ...
  const daily = await client.query(`
    SELECT date_trunc('day', created_at)::date AS day, count(*)::bigint AS row_count
    FROM public.security_events
    WHERE ${scoped}
    GROUP BY 1
    ORDER BY 1
...
async function main() {
  ...
  const apiRequestMode = String(args.get("api-request-mode") || DEFAULT_API_REQUEST_MODE);
  if (!API_REQUEST_MODES.has(apiRequestMode)) {
    throw new Error("--api-request-mode must be one of summary, exclude, all");
  }
  const batchSize = parseInteger(args.get("batch-size") ?? DEFAULT_BATCH_SIZE, "batch-size", 1, 10000);
  const loopback = isLoopbackDatabase(databaseUrl);
  if (!loopback && (!args.has("prod") || !args.has("force-production"))) {
    throw new Error("non-loopback evidence export requires --prod --force-production");
  }
  ...
  try {
    ...
    await client.query(`SELECT set_config('axtask.evidence_user_id', $1, true)`, [account.id]);

    const root = path.resolve(String(args.get("output-dir") || path.join(".backups", "evidence")));
    const exportDir = path.join(root, `account-evidence-${nowStamp()}-${safeFilePart(String(account.id).slice(0, 12))}`);
    ...
    for (const table of discovered) {
      ...
      if (SENSITIVE_TABLES_EXCLUDED.has(tableName)) {
        skippedTables.push({ table: tableName, reason: "ephemeral-or-secret-bearing" });
        continue;
      }
      ...
      const outputFile = path.join(exportDir, `${safeFilePart(tableName)}.jsonl`);
      const querySql = `SELECT * FROM public.${quoteIdent(tableName)} WHERE ${where}${orderByFor(columns)}`;
      files.push(await exportQueryToJsonl(client, { tableName, querySql, outputFile, batchSize }));
    }

    if (apiRequestSummary) {
      const summaryFile = path.join(exportDir, "security_events.api_request.summary.json");
      const summaryText = `${JSON.stringify(apiRequestSummary, null, 2)}\n`;
      writeFileSync(summaryFile, summaryText, { encoding: "utf8", mode: 0o600 });
      files.push({
    ...
    const manifest = {
      ...
      proofBoundary: "read-only database snapshot artifact; not a legal admissibility determination",
      ...
      apiRequestMode,
      apiRequestPolicy: apiRequestMode === "summary"
      ...
      bundleContentSha256: createHash("sha256").update(fileDigestInput, "utf8").digest("hex"),
    };

    const manifestFile = path.join(exportDir, "manifest.json");
    const manifestText = `${JSON.stringify(manifest, null, 2)}\n`;
    writeFileSync(manifestFile, manifestText, { encoding: "utf8", mode: 0o600 });
    const manifestSha = createHash("sha256").update(manifestText, "utf8").digest("hex");
    writeFileSync(
...
main().catch((error) => {
  console.error(`[account-evidence] FAILED: ${error instanceof Error ? error.message : String(error)}`);
  process.exit(1);
});
