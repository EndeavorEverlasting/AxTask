...
export function validateStatefulArchitecture(root = DEFAULT_ROOT) {
  ...
  if (ledger && ledgerSchema) validateAgainstSchema(ledger, ledgerSchema, ".ai/stateful-surface-ledger.json", errors);
  ...
  if (ledger) {
    ...
    if (!/\bone\s+named\s+migrationSeam\b/i.test(migrationRule)) errors.push("migrationAuthorizationRule must require one named migrationSeam");
    if (!/\bprovisional\b/i.test(migrationRule) || !/\bKEEP\b/.test(migrationRule)) errors.push("migrationAuthorizationRule must preserve provisional fail-closed KEEP semantics");
  ...
  for (const required of ["stateful-surface-ledger","stateful-architecture-workflow","stateful-architecture-skill","stateful-architecture-validator","stateful-architecture-report"]) if (!componentIds.has(required)) errors.push(`.ai/harness.json missing component ${required}`);
  ...
  for (const heading of ["## Trigger","## Inputs","## Factoring rules","## Execution loop","## Parallel lanes","## Validation","## Outputs","## Proof ceiling"]) if (!workflowText.includes(heading)) errors.push(`stateful architecture workflow missing ${heading}`);
  for (const phrase of ["KEEP is a valid final decision","one migration seam per sprint","application logic remains in code and domain contracts","do not choose a serverless provider"]) if (!workflowText.toLowerCase().includes(phrase.toLowerCase())) errors.push(`stateful architecture workflow missing guardrail: ${phrase}`);
  for (const heading of ["## Use when","## Required inputs","## Procedure","## Guardrails","## Outputs","## Proof rules"]) if (!skillText.includes(heading)) errors.push(`stateful architecture skill missing ${heading}`);
  for (const phrase of ["Stateful does not mean bad","provisional","KEEP","one seam","serverless provider"]) if (!humanLedger.toLowerCase().includes(phrase.toLowerCase())) errors.push(`human stateful ledger missing guardrail: ${phrase}`);
  for (const heading of ["## CURRENT BASELINE","## LEDGER CHANGES","## OWNED SEAM","## VALIDATION","## PROOF","## RISKS","## NEXT ACTION"]) if (!reportTemplate.includes(heading)) errors.push(`stateful architecture report template missing ${heading}`);
