...
export type MfaHandoffPersistContext = {
  /** Current session user's `role`, if signed in. */
  userRole: string | null | undefined;
  /** True while auth bootstrap is in flight on `/mfa/confirm`. */
  authLoading: boolean;
};

/**
 * Whether `/mfa/confirm` may call {@link storeMfaHandoffSession} for this purpose.
 * Admin step-up codes are only written when the viewer is a signed-in admin.
 */
export function shouldPersistMfaEmailHandoff(purpose: string, ctx: MfaHandoffPersistContext): boolean {
