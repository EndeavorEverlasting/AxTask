...
function readQueueForKey(key: string): OfflineTaskOp[] {
  ...
  try {
    ...
    if (!raw && key !== OFFLINE_TASK_QUEUE_STORAGE_KEY) {
      const legacy = localStorage.getItem(OFFLINE_TASK_QUEUE_STORAGE_KEY);
      // Never copy global legacy into a per-user key — another account could inherit user A's queue.
      if (legacy && userIdFromOfflineStorageKey(key) === null) {
        raw = legacy;
        try {
