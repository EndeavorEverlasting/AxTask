...
export type BrowserConnectionLike = {
  type?: string;
  effectiveType?: string;
  downlink?: number;
...
export function classifyNetworkConnection(input: {
  ...
  if (type && CELLULAR_TYPES.has(type)) {
    return {
      ...
      shouldWarn: true,
      warningMessage:
        "You appear to be on cellular data. AxTask may use your data plan until you reconnect to Wi-Fi.",
      effectiveType,
      downlink,
    };
  }
  ...
  if (effectiveType && CELLULAR_TYPES.has(effectiveType)) {
    return {
      ...
      shouldWarn: true,
      warningMessage:
        "You may be on metered data. AxTask may use your data plan until you reconnect to Wi-Fi.",
      effectiveType,
      downlink,
    };
  }
