...
export function Avatar({ name, size = "md" }: { name: string; size?: "sm" | "md" }) {
  ...
  return (
    <div
      className={`${dim} rounded-full bg-primary/15 text-primary font-semibold flex items-center justify-center shrink-0`}
    >
...
export function ProviderIcon({ provider, className = "h-5 w-5" }: { provider: string; className?: string }) {
  ...
  if (provider === "workos")
    return <ShieldCheck className={className + " text-indigo-600 dark:text-indigo-400"} />;
