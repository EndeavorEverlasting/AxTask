...
4. The nearest scoped rules and canonical document for the selected failure cluster.
5. package.json scripts, current tests, open PR collision surfaces, and recent commits.
6. Old plans and historical platform notes only when current evidence points to them.
...
Objective:
Recover current AxTask truth, identify the highest-value reproducible failure cluster that is not already owned by an open PR, repair its smallest shared root cause, add deterministic enforcement, validate the repair, and commit the result in the isolated GNHF worktree.
...
3. Read AGENTS.md and AGENT_GUARDRAILS.md, then load only the canonical document and tests governing the candidate cluster.
4. Inspect current open PRs and recent commits before choosing files, so the night run does not duplicate or collide with active work.
...
11. Run npm run perf:bundle only when the repair can affect client bundle size. Run npm run perf:api-replay only when it can affect API latency. Run the relevant deploy contract only when a non-deployment change legitimately touches its checked behavior; do not broaden into deployment work.
