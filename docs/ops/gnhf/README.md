...
This directory contains the compact AxTask runtime objective compiled from the repository harness and bounded night-shift rules.
...
From a clean, non-main AxTask branch, double-click:
...
The AxTask launcher expects the current AgentSwitchboard repository at:
...
when `-RepairControlPlane` is supplied or the installed provider launcher is absent. AgentSwitchboard PR #25 merged the Windows-safe provider route. The repair requires GNHF `0.1.42` or newer and verifies GNHF model support before installing the provider-routed launcher under `%LOCALAPPDATA%\AgentSwitchboard\GnhfFleet`.
...
1. dispatches Windows `.ps1` command shims through PowerShell;
2. requires GNHF `0.1.42` or newer and verified model support;
3. requires a compatible OpenCode runtime;
...
7. passes the exact model to GNHF;
