...
- replaced the vulnerable npm `xlsx` package with the API-compatible npm alias `xlsx: npm:@e965/xlsx@0.20.3`, preserving the six existing browser/server SheetJS import/export paths while avoiding the unpatched npm release; the alias repair was required to pass both the moderate production audit and TypeScript before commit;
...
- used three branch-only one-shot lock-repair workflows to regenerate `package-lock.json` from npm and require audit/type compatibility evidence before committing; all temporary workflows were deleted immediately after their bounded repair completed and are not part of the permanent release machinery;
