...
|----------------------|--------------|
| `tools/render/render-env-bootstrap.mjs`, `.env.render.example`, this checklist | `.env.render`, Render/env **export** files matching `*EnvFromRender.env`, and any file that contains real `DATABASE_URL`, OAuth secrets, `SESSION_SECRET`, `RESEND_API_KEY`, etc. |

The bootstrap script is only **logic**; it does not embed secrets. Generated values live in **gitignored** files (see root `.gitignore`: `.env.render`, backups, `*EnvFromRender.env`). That keeps recovery fast: clone the repo, run the generator, paste into a **new** Render service, and rotate credentials at each provider if something was compromised.
...
2. **Agents / no secret access:** `npm run render:env-placeholder -- --domain=YOUR_HOST` — writes `.env.render` with **placeholders** only (same shape as the template; fill or paste real values before deploy).
3. **Rotate machine-generated secrets in an existing file:** `npm run render:env-bootstrap -- --refresh-secrets-only --force` then copy the new `SESSION_SECRET`, `AUTH_AUDIT_PEPPER`, and `INVITE_CODE` (if present) into Render → Environment and redeploy.
4. **If an IdP was compromised:** create new OAuth client / WorkOS app keys in that vendor’s dashboard, update Render env, and ensure redirect URIs match `BASE_URL` (see reuse rules below).
5. **Pipe mode:** `npm run render:env-bootstrap -- --stdout --domain=YOUR_HOST > .env.render` — env bytes go to the file; messages and the paste guide go to **stderr** so they are not mixed into `.env.render`. The script **refuses** `--stdout` when stdout is an interactive terminal so values do not land in shell scrollback.
...
| `AUTH_AUDIT_PEPPER` | **`npm run render:env-bootstrap`** fills this from `.env.render.example`, or: `node -e "console.log(require('crypto').randomBytes(24).toString('base64url'))"` — must be **≥ 16** characters. **Required in production:** if unset, the server exits during bootstrap (`registerRoutes`). Unique per environment; rotate if compromised. |
...
AUTH_AUDIT_PEPPER=YOUR_RANDOM_STRING_AT_LEAST_16_CHARS
```

**`AUTH_AUDIT_PEPPER`:** Without this (or if shorter than 16 characters), **`NODE_ENV=production`** deploys fail immediately after `db:push` with **`[axtask:fatal] Server bootstrap failed`** — the process never listens, so **`/ready`** and login/MFA never come up. Set it in the same **Environment** list as `SESSION_SECRET`.
