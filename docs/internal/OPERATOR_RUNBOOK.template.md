...
1. `git clone` / `cd AxTask`. **`npm install`** verifies NodeWeaver under `services/nodeweaver/upstream/` (vendored). Optional: `npm run submodule:init` runs the same bootstrap script (legacy name).
...
| Rotate Render env file secrets | **`npm run render:env-bootstrap -- --refresh-secrets-only --force`** then update Render dashboard (`SESSION_SECRET`, `AUTH_AUDIT_PEPPER`, `INVITE_CODE` if present) |
