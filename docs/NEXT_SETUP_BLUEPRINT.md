...
Why this baseline:
- Managed Node hosting + simple deploy flow from GitHub
- No Replit lock-in
...
## B. Add task sounds ("Duolingo-style" urgency cue)

- [ ] Add optional sound preferences in user settings (`enabled`, `volume`, `quietHours`)
- [ ] Trigger sound cues on task overdue, task completion, and high-priority reminders
- [ ] Add mobile-safe audio fallback (respect browser autoplay restrictions)
- [ ] Include a global mute quick-toggle in UI

## C. Clock/alarm + notifications integration

- [ ] Add server-side scheduler for reminders (cron/queue)
- [ ] Create reminder entity (`taskId`, `fireAt`, `channel`, `status`)
- [ ] Start with in-app + email notifications, then add push/SMS later
- [ ] Support timezone-safe alarms for technicians in different locations

## D. Integrate NodeWeaver into one infrastructure/billing plane
...
3. **Day 5-7:** Sound cues + user preference controls
4. **Day 8-10:** Alarm/reminder scheduler + in-app/email notifications
5. **Day 11-14:** Contract hardening, integration spike, and go/no-go decision for single runtime
...
1. Add reminder schema + scheduler skeleton.
2. Add sound preference fields and UI toggle.
3. Add `NodeWeaver` contract test stubs in AxTask CI.
