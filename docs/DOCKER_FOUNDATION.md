...
**`services/nodeweaver/upstream`** is intentionally omitted from the AxTask image Docker build context (see **`.dockerignore`**). During that slim install, the Dockerfile sets **`AXTASK_BOOTSTRAP_ALLOW_MISSING_NODEWEAVER=1`** so **`repo-bootstrap.mjs`** skips the strict **`services/nodeweaver/upstream/Dockerfile`** check. Do not set that variable on a normal developer machine unless you are deliberately reproducing a slim image context; a normal clone already includes the vendored NodeWeaver tree.
