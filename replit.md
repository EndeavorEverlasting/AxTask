...
-   **Universal Voice Command System**: A global voice command bar (Ctrl+M) with a sophisticated server-side dispatching system that classifies intent (task creation, planner query, calendar command, navigation, search) and processes commands through dedicated engines (Calendar Engine, Planner Engine).
...
### Database Schema
Key tables include `users` (id, email, passwordHash, role, authProvider details, security info, ban status), `password_reset_tokens` (id, userId, tokenHash, expiry), `security_logs` (id, eventType, userId, targetUserId, ipAddress, details), and `tasks` (id, userId, date, time, activity, notes, urgency, impact, effort, priority, status, sortOrder, etc.). Sessions are managed by `connect-pg-simple`.
