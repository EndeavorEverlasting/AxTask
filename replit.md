...
-   **Interactive Tutorial**: A 14-step guided walkthrough (Ctrl+T to start) covering Dashboard, AI Planner, Task Form, Voice Commands, Classification & Compound Interest, Calendar, Analytics, Rewards Shop, Print Checklist, Import/Export, Google Sheets, and Keyboard Shortcuts. Uses yellow/green glows on sidebar links with an overlay tooltip. Engine: `client/src/hooks/use-tutorial.tsx`. UI: `client/src/components/tutorial-overlay.tsx`.
...
-   **Collaboration Rewards**: Users earn AxCoins for collaboration activities. Sharing a task with a collaborator earns 5 coins (once per task to prevent farming — dedup via `collaboration_share` transaction check). Completing a task that has collaborators earns an additional 8-coin collaboration bonus. Collaboration badges: Team Player (first share, +10 bonus coins), Collaborator (5 shares, +10 coins), Team Leader (25 shares, +10 coins). Engine: `server/coin-engine.ts` (`awardCoinsForSharing`, collaboration completion bonus in `awardCoinsForCompletion`). UI: Share dialog toast shows earned coins and badges.
