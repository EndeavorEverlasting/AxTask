...
-   **Data Migration Toolkit**: Full database export/import with referential integrity validation. Admin UI under Security Admin > Data Migration tab for full-database or per-user exports, file upload import with dry-run validation, and detailed result reports. User self-service export via `GET /api/account/export` (GDPR data portability). CLI script at `scripts/migrate.ts` for command-line backup/restore. Export format is database-agnostic JSON with metadata header and dependency-ordered table data. Import engine checks all FK references, skips existing records, and supports batch insertion. Engine: `server/migration/export.ts`, `server/migration/import.ts`. Routes: `POST /api/admin/export`, `GET /api/admin/export/:userId`, `POST /api/admin/import`, `POST /api/admin/import/validate`, `GET /api/account/export`.
